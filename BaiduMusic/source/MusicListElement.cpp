...
CMusicListElementUI::CMusicListElementUI()
	: m_pSend(NULL),
	//m_MusicPlayer(this),
	m_bPlayAnim(false),
	m_pPlay(NULL),
...
LPCTSTR CMusicListElementUI::GetClass() const
{
	return _T("MusicListElementUI");
}

void CMusicListElementUI::DoEvent(TEventUI& event)
{
...
int CMusicListElementUI::GetColumnIndex( POINT cPoint )
{
...
bool CMusicListElementUI::Activate()
{
...
void CMusicListElementUI::DrawItemBk(HDC hDC, const RECT& rcItem)
{
	ASSERT(m_pOwner);
...
void CMusicListElementUI::DrawButton(HDC hDC, const RECT& rcItem)
{
	
}
void CMusicListElementUI::DrawAnimate(HDC hDC, const RECT& rcItem)
{
	if(m_pAnimate->IsVisible() == false)
		return ;
	if( m_pOwner == NULL ) return;
	TListInfoUI* pInfo = m_pOwner->GetListInfo();

	LPCTSTR pStrModify = NULL;
	int m_rcTile = m_pTitle->GetWidth()+pInfo->rcTextPadding.left;
...
void CMusicListElementUI::DoPaint(HDC hDC, const RECT& rcPaint)
{
...
void CMusicListElementUI::CElementSetValue(MEDIA *m_Media)
{
...
MEDIA*  CMusicListElementUI::GetMedia()
{
	return m_pMedia;
}

void  CMusicListElementUI::SetActive(bool bActive)
{
	...
	if(m_bActive)
	{
		m_pManager->SetTimer(this,1,100);
...
bool  CMusicListElementUI::GetActive()
{
	return m_bActive;
}
...
void CMusicListElementUI::AnimatePlay(bool bPlay)
{
	if(m_bPlayAnim == bPlay)
		return;
