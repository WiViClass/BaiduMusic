...
public:
	...
	virtual void SetItemActive(int iIndex) = 0;
};
...
public:
	...
	//virtual void SetOwnList(CControlUI* pOwner) = 0;
    virtual bool IsSelected() const = 0;
    virtual bool Select(bool bSelect = true) = 0;
    virtual bool IsExpanded() const = 0;
    virtual bool Expand(bool bExpand = true) = 0;
    virtual void DrawItemText(HDC hDC, const RECT& rcItem) = 0;
	//virtual bool SetMouseSelect( bool bSelect ) = 0;    // 仅用来处理鼠标滑动选择
	virtual bool SetSelect( bool bSelect ) = 0;    // 因为要支持多选，CListContainerElementUI里面的select对于多选跟单选有点冲突，所以写了这个
...
public:
	...
	//修改LC
	void GetItemPos(POINT ptMouse); //判断位置如果距离边框一个像素那么判定离开，否则在边框内
	//如果在边框内那么判断是否ITEM数量能否充满界面充满则修改滚动条否则不修改

	void SetScrollPt( CScrollBarUI* m_VerticalScrollBar,CScrollBarUI* m_HorizontalScrollBar);
...
public:
    ...
    IListOwnerUI* GetOwner();
    void SetOwner(CControlUI* pOwner);
	//void SetOwnList(CControlUI* pOwner);
    void SetVisible(bool bVisible = true);
    void SetEnabled(bool bEnable = true);
