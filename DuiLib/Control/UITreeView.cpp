...
namespace DuiLib
{
	...
	CTreeNodeUI::CTreeNodeUI( CTreeNodeUI* _ParentNode /*= NULL*/ )
	{
		...
		m_dwItemHotTextColor	= 0x00000000;
		m_dwSelItemTextColor	= 0x00000000;
		m_dwSelItemHotTextColor	= 0x00000000;
		...
		m_pButton->SetFixedWidth(GetFixedHeight());
...
//2015.10.12
	...
	void CTreeNodeUI::SetFolder()
	{

		if(IsHasChild())
		{
			//if(pFolderButton->IsSelected())
	...
	bool CTreeNodeUI::Select( bool bSelect /*= true*/ )
	{
		...
		bool IsEmpty = mTreeNodes.IsEmpty();
		//修改LC
		//if(!IsHasChild())
		bool nRet = true;
		if(IsEmpty)
		{
			 nRet = CListContainerElementUI::Select(bSelect);
		}
	...
	bool CTreeNodeUI::SetSelect( bool bSelect /*= true*/ )
	{
		...
		if(IsEmpty)
		{
			nRet = CListContainerElementUI::SetSelect(bSelect);
		}
	...
	void CTreeNodeUI::SetAttribute( LPCTSTR pstrName, LPCTSTR pstrValue )
	{
		...
		else if(_tcscmp(pstrName, _T("buttonattr")) == 0 )
			m_pButton->ApplyAttributeList(pstrValue);
