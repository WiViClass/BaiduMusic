...
CListUI::CListUI() : m_pCallback(NULL),m_iCurActiveItem(-1), m_bScrollSelect(false), m_iCurSel(-1), m_iExpandedItem(-1)
{
...
bool CListUI::Add(CControlUI* pControl)
{
	...
	LPCTSTR pstrClassName=pControl->GetClass();
...
void CListUI::DoEvent(TEventUI& event)
{
	...
	if(event.Type > UIEVENT__MOUSEBEGIN && event.Type < UIEVENT_MOUSEHOVER && m_pList->m_bVScrollBarShow)
	{
	 ...
	 if( _tcsstr(event.pSender->GetClass(), _T("ListUI")) != NULL ) 
	 {
		 CDuiString sLog = _T(" List ");

		// sLog.Format(_T(" List : "),rc.top, rc.bottom,cxNeeded ,cyNeeded);
...
//修改LC：判断ITEM是否在边框上
void CListUI::GetItemPos(POINT ptMouse)
{
	//return TRUE;
	RECT m_Pos = GetPos();
	//POINT是相对位置
	POINT m_ptMouse = ptMouse;
	//判断是否在边上
	if(m_ptMouse.x <= m_Pos.left+5 || m_ptMouse.x >= m_Pos.right-10 || m_ptMouse.y >= m_Pos.bottom-5 || m_ptMouse.y <= m_Pos.top+20)
	{
	//if(m_ptMouse.x <=10 || m_ptMouse.y <=10 )
	//{
		m_pList->SetScrollVisible(false);
	}
	else
	{
		m_pList->SetScrollVisible(true);
	}

}
void CListUI::SetScrollPt( CScrollBarUI* m_VerticalScrollBar,CScrollBarUI* m_HorizontalScrollBar)
{
	//m_pVerticalScrollBar = m_VerticalScrollBar;
	//m_pHorizontalScrollBar = m_HorizontalScrollBar;
}
...
bool CListUI::SelectItem(int iIndex, bool bTakeFocus)
{
    ...
    if( m_iCurSel >= 0 ) {
        ...
        if( pControl != NULL) {
            IListItemUI* pListItem = static_cast<IListItemUI*>(pControl->GetInterface(_T("ListItem")));
            if( pListItem != NULL ) pListItem->SetSelect(false);
        }
...
void CListUI::SetItemActive(CControlUI* pControl ,int iIndex)
{
	//if()
...
void CListUI::SetItemActive(int iIndex)
{
	//if()
...
void CListBodyUI::SetScrollVisible(bool IsVisible)
{
	//return;
	...
	if(IsVisible ) 
	{
		...
		{
			if(FALSE ==m_pVerticalScrollBar->IsVisible())
			{
...
void CListBodyUI::SetPos(RECT rc)
{
	...
	//sLog.Format(_T(" ScrollBar : top.buttom: %d.%d cx.cy : %d.%d"),rc.top, rc.bottom,cxNeeded ,cyNeeded);
...
void CListHeaderItemUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("textpadding")) == 0 || _tcscmp(pstrName, _T("text-pos")) == 0) {
		RECT rcTextPadding = { 0 };
		LPTSTR pstr = NULL;
		rcTextPadding.left = _tcstol(pstrValue, &pstr, 10);  ASSERT(pstr);    
		rcTextPadding.top = _tcstol(pstr + 1, &pstr, 10);    ASSERT(pstr);    
...
// 为了下拉框不支持多选
bool CListElementUI::SetSelect( bool bSelect )
{
	if( !IsEnabled() ) return false;
	if( bSelect == m_bSelected ) return true;
	m_bSelected = bSelect;
...
void  CListElementUI::SetActive(bool bActive)
{
	m_bActive = bActive;
}
bool  CListElementUI::GetActive()
{
	return m_bActive;
}
...
void CListElementUI::DoEvent(TEventUI& event)
{
	...
	//修改LC：添加鼠标右键消息类型，将原来子类的消息放到父类发送
	if( event.Type == UIEVENT_RBUTTONDOWN )
	{
		if( IsEnabled() ) {
			m_pManager->SendNotify(this, DUI_MSGTYPE_RBUTTONMENU);
			Select();
			Invalidate();
		}
		return;
...
void CListLabelElementUI::DoEvent(TEventUI& event)
{
	...
	//修改LC：更改item的右键消息类型，将其改为发送DUI_MSGTYPE_RBUTTONMENU消息
	//由其父类CListElementUI发送
	...
	//修改LC：传递UIEVENT_MOUSEMOVE到LIST用来判断滚动条
    if( event.Type == UIEVENT_MOUSEMOVE ) 
    {
      //  return;
    }
...
SIZE CListTextElementUI::EstimateSize(SIZE szAvailable)
{
    ...
    if( cXY.cy == 0 && m_pManager != NULL ) {
		//修改LC：修改文本列表控件的高度
        cXY.cy = m_pManager->GetFontInfo(pInfo->nFont)->tm.tmHeight + 18;
...
// 为了下拉框不支持多选
bool CListContainerElementUI::SetSelect( bool bSelect )
{
	if( !IsEnabled() ) return false;
	if( bSelect == m_bSelected ) return true;
	m_bSelected = bSelect;
...
void  CListContainerElementUI::SetActive(bool bActive)
{
	m_bActive = bActive;
}
bool  CListContainerElementUI::GetActive()
{
	return m_bActive;
}
