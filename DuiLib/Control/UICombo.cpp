...
void CComboUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_MOUSEMOVE  || event.Type == UIEVENT_ITEMMOUSEMOVE )
    {
        return;
    }
...
{
    if( _tcscmp(pstrName, _T("textpadding")) == 0 || _tcscmp(pstrName, _T("text-pos")) == 0) {
...
void CComboUI::SetItemActive(int iIndex)
{
	//if()
